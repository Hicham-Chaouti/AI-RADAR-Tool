...
Tasks:
    ...
    3. Add benefits + data_prerequisites
...
# ═══════════════════════════════════════════════════════════
# TASK 3 — Add benefits + data_prerequisites
# ═══════════════════════════════════════════════════════════
...
async def add_data_prerequisites(
    records: list[dict[str, Any]], mistral_client: Mistral
) -> int:
    """Add data_prerequisites via Mistral. Skips already-enriched records. Returns count added."""
    # Count how many already have prerequisites (from a previous run)
    already_done = sum(1 for r in records if r.get("data_prerequisites"))
    if already_done:
        log.info(f"  {already_done} records already have data_prerequisites (resuming)")
    ...
    for idx, record in enumerate(records):
        ...
        if record.get("data_prerequisites"):
            continue
...
"""
        try:
            ...
            if isinstance(parsed, list):
                ...
                prereqs = None
                for key in ("data_prerequisites", "prerequisites", "data", "items"):
            ...
            if prereqs:
                record["data_prerequisites"] = prereqs
                count += 1

        except Exception as e:
            if llm_calls < 5:
                log.warning(f"  Failed data_prerequisites for '{record.get('title', '?')[:40]}': {e}")
        ...
        if llm_calls % 100 == 0:
            log.info(f"  data_prerequisites: {llm_calls} LLM calls done, {count}/{total} enriched")
...
async def main() -> None:
    ...
    if ENRICHED_FILE.exists():
        ...
        already_enriched = sum(1 for r in records if r.get("data_prerequisites"))
        log.info(f"Resuming from checkpoint: {len(records)} records ({already_enriched} already have data_prerequisites)")
    ...
    # ── TASK 3: Add benefits + data_prerequisites ──
    log.info(f"\n{'=' * 60}")
    log.info("TASK 3: Adding benefits + data_prerequisites")
    log.info(f"{'=' * 60}")
    benefits_count = add_benefits(records)
    log.info(f"  Benefits added: {benefits_count} (from measurable_benefit)")
    save_checkpoint(records)
    log.info("  Starting data_prerequisites enrichment (LLM calls)...")
    prereqs_count = await add_data_prerequisites(records, mistral_client)
