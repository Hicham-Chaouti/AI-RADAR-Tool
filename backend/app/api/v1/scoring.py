...
async def score_use_cases(
    ...
    try:
        ...
        # 8. Assign ranks + generate justifications (parallelize for speed)
        async def _fill_justification(rank: int, sr: ScoringResult):
            sr.rank = rank
            uc = use_cases_map[sr.use_case_id]
            sr.justification = await llm.generate_justification(uc, session)

        tasks = [
            _fill_justification(i + 1, sr) 
        ...
        # Re-raise FastAPI HTTPExceptions (like 404 or 503 from wait_for_model)
        raise
    except Exception as e:
        log.error(f"Scoring pipeline failed: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500,
